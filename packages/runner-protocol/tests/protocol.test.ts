...
describe('@textmode/runner-protocol', () => {
	...
	it('validates current parent messages', () => {
		...
		expect(isParentMessage({ type: 'RESET_RUNTIME', requestId: 'reset_1', code: 't.draw(() => {})' })).toBe(true);
	...
	it('rejects malformed parent payloads', () => {
		...
		expect(
			isParentMessage({ type: 'AUDIO_DATA', fft: [1, 2], waveform: new Uint8Array([128]), timestamp: 1 })
		).toBe(false);
